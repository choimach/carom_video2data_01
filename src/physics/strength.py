...
프로 1335개 플레이의 초속을 이 자로 읽으면:
    ...
    25%  강도 2.0     50%  강도 2.9     75%  강도 3.7     90%  강도 4.8

실전은 강도 2~5에 들어간다. 강도 8 이상은 이론상의 값이고 (초속 12 m/s가
넘는다) 데이터에 그렇게 친 사람은 없다.
"""
...
_STRENGTH = np.array([0.31, 0.72, 1.13, 1.70, 2.22, 2.73, 3.13, 3.51, 3.93,
                      4.20, 4.89, 5.66, 6.25, 7.14, 8.03, 9.37, 10.91])
_SPEED_MM_S = np.array([600, 900, 1200, 1600, 2000, 2400, 2800, 3200, 3600,
                        4000, 4800, 5600, 6400, 8000, 10000, 14000, 20000], dtype=float)
