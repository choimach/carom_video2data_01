...
# 2026-09-25에 **우리 캐롬 영상으로 갈아 끼웠다.** 그 전 값
# (0, 16.7, 33.7, 41.8, 50.1, 55.4, 63.0, 70.6, 80.1, 90)은 출처가 적혀 있지
# 않았고 마디 각도가 논문에서 디지타이즈한 모양이었다 — 그런 실험은 대개
# 포켓볼/스누커 테이블에서 한다. 우리 자료는 **가열된 캐롬 테이블**의 프로
# 영상 6,115건이고(`tools/measure_rebound.py`), 선수가 치는 당구장도 가열
# 테이블이므로 같은 세계다.
#
# 안 본 경기 320판으로 잰 결과 (`tools/check_after_cushion.js`):
#
#             2쿠션    3쿠션    2팁 회전이득
#   옛 표     241mm    383mm      9.2도
#   새 표     186mm    263mm      8.0도
#   영상                          10.8도
#
# 2·3쿠션이 23%·31% 줄었다. 회전 이득은 1.2도 멀어졌지만, 그 자는 영상의
# 10.8도가 **알 수 없는 팁**에서 나온 값이라 약한 자다.
#
# ⚠️ 첫 마디는 0으로 고정한다 — 수직으로 들어가면 수직으로 나와야 한다.
# 영상의 20.1도는 ±6도 창에 얕은 입사가 섞인 것이지 0도의 값이 아니다.
#
# ⚠️ 이 값에는 **회전이 섞여 있다.** 프로가 73%를 정회전으로 치므로 넓은 쪽으로
# 치우쳐 있을 것이고, 표는 무회전 기준이어야 한다. 회전이 적은 건만 골라 다시
# 재려 했으나 app-data.json에 inning/shot이 없어 english을 붙일 수 없었다.
# **그 조인을 만들면 이 표를 제대로 다시 잴 수 있다** — 남은 일이다.
#
# ⚠️ 장쿠션과 단쿠션을 따로 두는 것도 재 봤다. 영상에서 차이는 분명하지만
# (얕은 입사에서 12도) **궤적 예측은 나아지지 않았다** — 두 표본에서 한 번은
# 같고 한 번은 2쿠션만 낫고 3쿠션은 나빴다. 측정되지 않은 이득에 복잡도를
# 남기지 않는다. `ref/terms.md`의 "쿠션의 성질" 참조.
REBOUND_OUT = np.array([0.0, 25.6, 40.8, 46.1, 53.8, 58.6, 65.2, 71.8, 80.1, 90.0])
